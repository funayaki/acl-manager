			...
			foreach($ctrl_infos as $ctrl_info)
			{
		    	...
		    	foreach($roles as $role)
		    	{
		    	    ...
		    	    if(isset($ctrl_info['permissions'][$role[$role_model_name][$role_pk_name]]))
		    	    {
    		    		...
    		    		{
    		    			$this->Js->buffer('register_role_toggle_right(true, "' . $this->Url->build('/') . '", "right__' . $role[$role_model_name][$role_pk_name] . '_' . $controller_name . '_' . $ctrl_info['name'] . '", "' . $role[$role_model_name][$role_pk_name] . '", "", "' . $controller_name . '", "' . $ctrl_info['name'] . '")');
        		    
    		    			echo $this->Html->image('/acl/img/design/tick.png', array('class' => 'pointer'));
    		    		}
    		    		else
    		    		{
    		    			$this->Js->buffer('register_role_toggle_right(false, "' . $this->Url->build('/') . '", "right__' . $role[$role_model_name][$role_pk_name] . '_' . $controller_name . '_' . $ctrl_info['name'] . '", "' . $role[$role_model_name][$role_pk_name] . '", "", "' . $controller_name . '", "' . $ctrl_info['name'] . '")');
        	        ...
        	        foreach($roles as $role)
    		    	{
    		    	    ...
    		    	    if(isset($ctrl_info['permissions'][$role[$role_model_name][$role_pk_name]]))
    		    	    {
        		    		if($method['permissions'][$role[$role_model_name][$role_pk_name]] == 1)
        		    		{
        		    		    ...
        		    		    $this->Js->buffer('register_role_toggle_right(true, "' . $this->Url->build('/') . '", "right_' . $plugin_name . '_' . $role[$role_model_name][$role_pk_name] . '_' . $plugin_ctrler_name . '_' . $method['name'] . '", "' . $role[$role_model_name][$role_pk_name] . '", "' . $plugin_name . '", "' . $plugin_ctrler_name . '", "' . $method['name'] . '")');
        		    		...
        		    		else
        		    		{
        		    		    ...
        		    		    $this->Js->buffer('register_role_toggle_right(false, "' . $this->Url->build('/') . '", "right_' . $plugin_name . '_' . $role[$role_model_name][$role_pk_name] . '_' . $plugin_ctrler_name . '_' . $method['name'] . '", "' . $role[$role_model_name][$role_pk_name] . '", "' . $plugin_name . '", "' . $plugin_ctrler_name . '", "' . $method['name'] . '")');
